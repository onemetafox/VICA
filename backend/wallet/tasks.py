...
@app.task
def check_unconfirmed_external_transactions():
    """ Check unconfirmed transactions from the main wallet to the another address (Withdraw)
    
    """

    unconfirmed_eth_transactions = ETHTransaction.objects.filter(from_address=settings.ETHEREUM_WALLET_ADDRESS, status=ETHTransaction.Status.PENDING)

    for transaction in unconfirmed_eth_transactions:
        transaction.update_info()

    unconfirmed_btc_transactions = BTCTransaction.objects.filter(from_address=settings.BITCOIN_WALLET_ADDRESS, status=BTCTransaction.Status.PENDING)
...
@app.task
def check_for_0_02_eth_in_all_wallets():
...
def check_for_new_eth_transactions(self, address):
    ...
    if result.ok:
        eth_in_transactions = [item for item in result.json()['result'] if item['functionName'] == "" and item['to'].lower() == eth_wallet.address.lower() and item['from'].lower() != settings.ETHEREUM_WALLET_ADDRESS.lower()]
        for tx in eth_in_transactions:
            if ETHTransaction.objects.filter(hash=tx['hash']).exists():
                continue
            
            transaction = ETHTransaction.objects.create(wallet=eth_wallet, hash=tx['hash'], recipient_address=eth_wallet.address, from_address=tx['from'], currency=ETHTransaction.Currency.ETHEREUM, status=ETHTransaction.Status.CONFIRMED, amount=w3.fromWei(int(tx['value']), 'ether'), timestamp=tx['timeStamp'])
    ...
    if result.ok:
        usdt_in_transactions = [item for item in result.json()['result'] if item['to'].lower() == eth_wallet.address.lower()]
        for tx in usdt_in_transactions:
            if ETHTransaction.objects.filter(hash=tx['hash']).exists():
                continue
            
            amount = Decimal(int(tx['value']) / 1000000)
    ...
    if result.ok:
        vica_in_transactions = [item for item in result.json()['result'] if item['to'].lower() == eth_wallet.address.lower()]
        for tx in vica_in_transactions:
            if ETHTransaction.objects.filter(hash=tx['hash']).exists():
                continue
            
            amount = Decimal(int(tx['value']) / 1000000000000000000)
